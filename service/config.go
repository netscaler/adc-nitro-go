...
func (c *NitroClient) AddResourceReturnBody(resourceType string, name string, resourceStruct interface{}) ([]byte, error) {
	...
	if !contains(doNotPrintResources, resourceType) {
		log.Printf("[TRACE] nitro-go: Resourcejson is " + string(resourceJSON))
	}
...
func (c *NitroClient) UpdateResource(resourceType string, name string, resourceStruct interface{}) (string, error) {
	...
	if c.ResourceExists(resourceType, name) == true {
		...
		log.Printf("[DEBUG] nitro-go: UpdateResource: Resourcejson is " + string(resourceJSON))
...
func (c *NitroClient) ChangeResource(resourceType string, name string, resourceStruct interface{}) (string, error) {
	...
	if c.ResourceExists(resourceType, name) == true {
		...
		log.Printf("[DEBUG] nitro-go: ChangeResource: Resourcejson is " + string(resourceJSON))
...
func (c *NitroClient) DeleteResource(resourceType string, resourceName string) error {
	...
	if err == nil { // resource exists
		log.Printf("[DEBUG] nitro-go: DeleteResource Found resource of type %s: %s", resourceType, resourceName)
		_, err = c.deleteResource(resourceType, resourceName)
		if err != nil {
			log.Printf("[ERROR] nitro-go: Failed to delete resourceType %s: %s, err=%s", resourceType, resourceName, err)
			return err
		}
	} else {
		log.Printf("[INFO] nitro-go: DeleteResource: Resource %s already deleted ", resourceName)
	}
	return nil
}
...
func (c *NitroClient) DeleteResourceWithArgs(resourceType string, resourceName string, args []string) error {
	...
	if err == nil { // resource exists
		log.Printf("[INFO] nitro-go: DeleteResource found resource of type %s: %s", resourceType, resourceName)
		...
		log.Printf("[INFO] nitro-go: Resource %s already deleted ", resourceName)
	}
	return nil
}
...
func (c *NitroClient) UnbindResource(boundToResourceType string, boundToResourceName string, boundResourceType string, boundResourceName string, bindingFilterName string) error {
	...
	if c.ResourceExists(boundToResourceType, boundToResourceName) == false {
		log.Printf("[INFO] nitro-go: Unbind: BoundTo Resource %s of type %s does not exist", boundToResourceType, boundToResourceName)
		return nil
	}

	if c.ResourceExists(boundResourceType, boundResourceName) == false {
		log.Printf("[INFO] nitro-go: Unbind: Bound Resource %s of type %s does not exist", boundResourceType, boundResourceName)
		return nil
	}

	_, err := c.unbindResource(boundToResourceType, boundToResourceName, boundResourceType, boundResourceName, bindingFilterName)
	if err != nil {
		return fmt.Errorf("[ERROR] nitro-go: Failed to unbind  %s:%s from %s:%s, err=%s", boundResourceType, boundResourceName, boundToResourceType, boundToResourceName, err)
	}
...
func (c *NitroClient) ResourceExists(resourceType string, resourceName string) bool {
	_, err := c.listResource(resourceType, resourceName)
	if err != nil {
		log.Printf("[INFO] nitro-go: No %s %s found", resourceType, resourceName)
		return false
	}
	log.Printf("[INFO] nitro-go: %s %s is already present", resourceType, resourceName)
	return true
}
...
func (c *NitroClient) FindResourceArray(resourceType string, resourceName string) ([]map[string]interface{}, error) {
	...
	if err != nil {
		log.Printf("[WARN] nitro-go: FindResourceArray: No %s %s found", resourceType, resourceName)
		return nil, fmt.Errorf("[INFO] nitro-go: FindResourceArray: No resource %s of type %s found", resourceName, resourceType)
	}
	if err = json.Unmarshal(result, &data); err != nil {
		log.Printf("[ERROR] nitro-go: FindResourceArray: Failed to unmarshal Netscaler Response!")
	...
	if !ok || rsrcs == nil {
		log.Printf("[WARN] nitro-go: FindResourceArray No %s type with name %s found", resourceType, resourceName)
		return nil, fmt.Errorf("[INFO] nitro-go: FindResourceArray: No resource %s of type %s found", resourceName, resourceType)
	}
...
func (c *NitroClient) FindFilteredResourceArray(resourceType string, filter map[string]string) ([]map[string]interface{}, error) {
	...
	if err != nil {
		log.Printf("[WARN] nitro-go: FindFilteredResourceArray: No %s found matching filter %s", resourceType, filter)
	...
	if err = json.Unmarshal(result, &data); err != nil {
		log.Println("[ERROR] nitro-go: FindFilteredResourceArray: Failed to unmarshal Netscaler Response!")
	...
	rsrcs, ok := data[resourceType]
	if !ok || rsrcs == nil {
		log.Printf("[WARN] nitro-go: FindFilteredResourceArray No %s type matching filter %s found", resourceType, filter)
...
func (c *NitroClient) FindResource(resourceType string, resourceName string) (map[string]interface{}, error) {
	...
	result, err := c.listResource(resourceType, resourceName)
	if err != nil {
		log.Printf("[WARN] nitro-go: FindResource: No %s %s found", resourceType, resourceName)
		return nil, fmt.Errorf("[INFO] nitro-go: FindResource: No resource %s of type %s found", resourceName, resourceType)
	}
	if err = json.Unmarshal(result, &data); err != nil {
		log.Printf("[ERROR] nitro-go: FindResource: Failed to unmarshal Netscaler Response!")
	...
	if !ok || rsrc == nil {
		log.Printf("[WARN] nitro-go: FindResource No %s type with name %s found", resourceType, resourceName)
		return nil, fmt.Errorf("[INFO] nitro-go: FindResource: No resource %s of type %s found", resourceName, resourceType)
	}
	...
	default:
		log.Printf("[WARN] nitro-go: FindResource Unable to determine type of response")
...
func (c *NitroClient) FindResourceArrayWithParams(findParams FindParams) ([]map[string]interface{}, error) {
	...
	log.Printf("[TRACE] nitro-go: FindResourceArrayWithParams: url is %s", url)
	result, httpErr := c.doHTTPRequest("GET", url, bytes.NewBuffer([]byte{}), readResponseHandler)
	log.Printf("[TRACE] nitro-go: FindResourceArrayWithParams: HTTP GET result: %v", string(result))
	log.Printf("[TRACE] nitro-go: FindResourceArrayWithParams: HTTP GET error: %v", httpErr)
	...
	if httpErr != nil {
		if !strings.Contains(httpErr.Error(), "404") {
			...
			log.Printf("[DEBUG] nitro-go: FindResourceArrayWithParams: Ignoring 404 http status. %s", httpErr.Error())
		}
	}
	...
	if err != nil {
		return nil, fmt.Errorf("[ERROR] nitro-go: FindResourceArrayWithParams: JSON unmarshal error: %s", err.Error())
	}
	...
	if missingErrcode != 0 && missingErrcode == errorcode {
		log.Printf("[DEBUG] nitro-go: FindResourceArrayWithParams: resource missing error code returned %d", errorcode)
		return emptyRetval, nil
	}
	// Fallthrough

	if errorcode != 0 {
		return nil, fmt.Errorf("[ERROR] nitro-go: FindResourceArrayWithParams: non zero errorcode %d", errorcode)
	}
	// Fallthrough
	...
	if !ok {
		...
		log.Printf("[DEBUG] nitro-go: FindResourceArrayWithParams: resource key missing %s.", findParams.ResourceType)
		return emptyRetval, nil
	}
	// Falthrough

	log.Printf("[DEBUG] nitro-go: FindResourceArrayWithParams: retrieved NITRO object: %v", resourceData)
...
func (c *NitroClient) FindAllResources(resourceType string) ([]map[string]interface{}, error) {
	...
	if err != nil {
		log.Printf("[INFO] nitro-go: FindAllResources: No %s objects found", resourceType)
		return make([]map[string]interface{}, 0, 0), nil
	}
	if err = json.Unmarshal(result, &data); err != nil {
		log.Printf("[ERROR] nitro-go: FindAllResources: Failed to unmarshal Netscaler Response!")
		return nil, fmt.Errorf("[ERROR] nitro-go: FindAllResources: Failed to unmarshal Netscaler Response: of type %s", resourceType)
	}
	rsrcs, ok := data[resourceType]
	if !ok || rsrcs == nil {
		log.Printf("[INFO] nitro-go: FindAllResources: No %s found", resourceType)
		return make([]map[string]interface{}, 0, 0), nil
	}
	resources := data[resourceType].([]interface{})
...
func (c *NitroClient) ResourceBindingExists(resourceType string, resourceName string, boundResourceType string, boundResourceFilterName string, boundResourceFilterValue string) bool {
	...
	if err != nil {
		log.Printf("[INFO] nitro-go: ResourceBindingExists: No %s %s to %s %s binding found", resourceType, resourceName, boundResourceType, boundResourceFilterValue)
		return false
	}

	var data map[string]interface{}
	if err := json.Unmarshal(result, &data); err != nil {
		log.Printf("[ERROR] nitro-go: ResourceBindingExists: Failed to unmarshal Netscaler Response!")
		return false
	}
	...
	log.Printf("[INFO] nitro-go: ResourceBindingExists: %s of type  %s is bound to %s type and name %s", resourceType, resourceName, boundResourceType, boundResourceFilterValue)
	return true
}
...
func (c *NitroClient) FindBoundResource(resourceType string, resourceName string, boundResourceType string, boundResourceFilterName string, boundResourceFilterValue string) (map[string]interface{}, error) {
	result, err := c.listBoundResources(resourceName, resourceType, boundResourceType, boundResourceFilterName, boundResourceFilterValue)
	if err != nil {
		log.Printf("[INFO] nitro-go: FindBoundResource: No %s %s to %s %s binding found", resourceType, resourceName, boundResourceType, boundResourceFilterValue)
	...
	var data map[string]interface{}
	if err := json.Unmarshal(result, &data); err != nil {
		log.Printf("[ERROR] nitro-go: FindBoundResource: Failed to unmarshal Netscaler Response!")
...
func (c *NitroClient) FindAllBoundResources(resourceType string, resourceName string, boundResourceType string) ([]map[string]interface{}, error) {
	...
	if err != nil {
		log.Printf("[INFO] nitro-go: FindAllBoundResources: No %s %s to %s  binding found", resourceType, resourceName, boundResourceType)
		return nil, fmt.Errorf("[ERROR] nitro-go: No %s %s to %s binding found, err=%s", resourceType, resourceName, boundResourceType, err)
	}

	var data map[string]interface{}
	if err := json.Unmarshal(result, &data); err != nil {
		log.Printf("[ERROR] nitro-go: FindAllBoundResources: Failed to unmarshal Netscaler Response!")
...
func (c *NitroClient) EnableFeatures(featureNames []string) error {
	...
	*/
	...
	if err != nil {
		log.Printf("[ERROR] nitro-go: EnableFeatures: Failed to marshal features to JSON")
		return fmt.Errorf("[ERROR] nitro-go: Failed to marshal features to JSON")
	}

	_, err = c.enableFeatures(featureJSON)
	if err != nil {
		return fmt.Errorf("[ERROR] nitro-go: Failed to enable feature %v", err)
	}
	return nil
}
...
func (c *NitroClient) DisableFeatures(featureNames []string) error {
	...
	*/
	...
	featureJSON, err := JSONMarshal(featureStruct)
	if err != nil {
		log.Printf("[ERROR] nitro-go: DisableFeatures: Failed to marshal features to JSON")
		return fmt.Errorf("[ERROR] nitro-go: Failed to marshal features to JSON")
	}
...
func (c *NitroClient) EnableModes(modeNames []string) error {
	...
	*/
	...
	if err != nil {
		log.Printf("[ERROR] nitro-go: EnableModes: Failed to marshal modes to JSON")
...
func (c *NitroClient) ListEnabledModes() ([]string, error) {
	...
	var data map[string]interface{}
	if err = json.Unmarshal(bytes, &data); err != nil {
		log.Printf("[ERROR] nitro-go: ListEnabledModes: Failed to unmarshal Netscaler Response!")
		return []string{}, fmt.Errorf("[ERROR] nitro-go: Failed to unmarshal Netscaler Response to list Modes")
	}
	mode, ok := data["nsmode"]
	if !ok || mode == nil {
		log.Printf("No modes found")
		return []string{}, fmt.Errorf("[ERROR] nitro-go: No modes found")
	}
...
func (c *NitroClient) SaveConfig() error {
	...
	*/
	...
	if err != nil {
		log.Printf("[ERROR] nitro-go: SaveConfig: Failed to marshal save config to JSON")
...
func (c *NitroClient) ClearConfig() error {
	...
	*/
	...
	if err != nil {
		log.Printf("[ERROR] nitro-go: ClearConfig: Failed to marshal clear config to JSON")
